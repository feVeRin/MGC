...
ori_root = './mfcc_ori'+'/' + dataset + '_mfcc_ori'
index = 0
# print(genre)
...
# def feature_extraction(path):
#         y = librosa.load(path, sr=None)[0]
#         S = librosa.feature.melspectrogram(x, sr=44100, n_mels=128)
#         log_S = librosa.power_to_db(S, ref=np.max)
#         mfcc = librosa.feature.mfcc(S=log_S, n_mfcc=100)
...
### Create mfcc Train data from music files
...
for k in range(len(img1_folder)):
    ...
    np.save(save_path + '_mfcc/' + npy_name, layer4)
